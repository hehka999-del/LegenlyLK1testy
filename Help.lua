...
LangButton.Position = UDim2.new(1,-85,0,15)
...
IslandFrame.Size = UDim2.fromOffset(172,44)
...
IslandLogo.Position = UDim2.fromOffset(6,6)
...
local function RunOrbitFling(target,isVoidMode)
    ...
    task.spawn(function()
        ...
        if ToggleControllers["Fling"] then
            pcall(function()
                ToggleControllers["Fling"](false)
            end)
        end
...
-- Unified Fling toggle.
CreateToggle(TrollPage,"Fling",function(state)
    if state then
    ...
    if tr and mr then
        Notify(
            ...
            target.DisplayName
                ...
                .. "m • HP "
                .. (th and math.floor(th.Health) or 0),
            CurrentTheme.Accent
        )
    end
end)
...
CreateInfoLabel(
    TargetPage,
    "Choose the player here. Persistent target actions are controlled from the Troll tab."
)
