...
SplashTitle.Position = UDim2.fromScale(0.5,0.525)
...
SplashSub.Position = UDim2.fromScale(0.5,0.595)
...
SplashStatus.Position = UDim2.fromScale(0.5,0.665)
...
IslandFrame.Size = UDim2.fromOffset(210,52)
...
IslandLogo.Position = UDim2.fromOffset(9,7)
IslandLogo.Size = UDim2.fromOffset(38,38)
...
IslandText.Position = UDim2.fromOffset(56,0)
IslandText.Size = UDim2.new(1,-66,1,0)
...
local function CreateToggle(parent,textKey,callback,default)
    ...
    local glowHalo = Instance.new("UIStroke")
    glowHalo.Thickness = 4
    glowHalo.Transparency = 1
    glowHalo.Color = CurrentTheme.Accent
    glowHalo.Parent = frame
    RegisterForRecolor(glowHalo,"Color")
    ...
    local controller = function(value)
        ...
        TweenService:Create(
            ...
            {Transparency = state and 0.12 or 1, Thickness = state and 1.6 or 1}
        ):Play()
        TweenService:Create(
            glowHalo,
            TweenInfo.new(0.24,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),
            {Transparency = state and 0.72 or 1, Thickness = state and 4 or 4}
        ):Play()
        callback(state)

        if state then
            task.spawn(function()
                while frame.Parent and state do
                    local a = TweenService:Create(glowHalo, TweenInfo.new(0.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut), {Transparency = 0.84, Thickness = 3.2})
                    local b = TweenService:Create(glowStroke, TweenInfo.new(0.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut), {Transparency = 0.24, Thickness = 1.35})
                    a:Play(); b:Play(); a.Completed:Wait()
                    if not frame.Parent or not state then break end
                    local c = TweenService:Create(glowHalo, TweenInfo.new(0.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut), {Transparency = 0.68, Thickness = 4})
                    local d = TweenService:Create(glowStroke, TweenInfo.new(0.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut), {Transparency = 0.10, Thickness = 1.7})
                    c:Play(); d:Play(); c.Completed:Wait()
                end
            end)
        end
    end
...
local function CreateButton(parent,textKey,callback)
    ...
    -- Ordinary buttons intentionally have NO glow.
...
-- Forward declarations: Stepped must use the LOCAL noclip functions.
local EnableNoclip
local DisableNoclip
...
UserInputService.JumpRequest:Connect(function()
    if TrollState.Fly then
        TrollState._FlyUpUntil = os.clock() + 0.28
    end
end)
...
local function ApplySmoothFly()
    ...
    if UserInputService:IsKeyDown(Enum.KeyCode.Space) or os.clock() < TrollState._FlyUpUntil or hum.Jump then
        velocity +=
            Vector3.new(
                0,
                TrollState.FlySpeed,
...
local function CreateThemeDropdown(parent)
    ...
    mainLabel.Name = "CurrentThemeLabel"
    mainLabel.BackgroundTransparency = 1
    mainLabel.Position = UDim2.fromOffset(48,0)
    mainLabel.Size = UDim2.new(1,-82,1,0)
    mainLabel.Font = Enum.Font.GothamBold
    mainLabel.TextSize = 12
    mainLabel.TextColor3 = Color3.fromRGB(244,244,248)
    mainLabel.TextXAlignment = Enum.TextXAlignment.Left
    mainLabel.ZIndex = 203
    ...
    optionsStroke.Transparency = 0.18
