...
local RecolorQueue = {}
local TextElements = {}
local AnimatedGlowStrokes = {}

local function RegisterAnimatedGlow(stroke, minTransparency, maxTransparency, speed)
    AnimatedGlowStrokes[stroke] = {
        Min = minTransparency,
        Max = maxTransparency,
        Speed = speed or 1.0,
...
-- Subtle breathing glow. It stays soft so the outline remains readable instead of flashing.
task.spawn(function()
    local clock = 0
    while task.wait(0.045) do
        clock += 0.045
        for stroke, data in pairs(AnimatedGlowStrokes) do
            if not stroke or not stroke.Parent then
                AnimatedGlowStrokes[stroke] = nil
            else
                local wave = (math.sin(clock * data.Speed * math.pi * 2 + data.Phase) + 1) * 0.5
                stroke.Transparency = data.Max + (data.Min - data.Max) * wave
            end
        end
    end
end)
...
SplashLogo.Size = UDim2.fromOffset(104,104)
...
SplashTitle.Position = UDim2.fromScale(0.5,0.475)
...
SplashSub.Name = "SplashSubtitle"
SplashSub.AnchorPoint = Vector2.new(0.5,0.5)
SplashSub.Position = UDim2.fromScale(0.5,0.535)
...
SplashStatus.Name = "SplashStatus"
SplashStatus.AnchorPoint = Vector2.new(0.5,0.5)
SplashStatus.Position = UDim2.fromScale(0.5,0.605)
...
IslandFrame.Position = UDim2.fromScale(0.5,0.012)
IslandFrame.Size = UDim2.fromOffset(118,42)
...
islandGlow.Thickness = 5
islandGlow.Transparency = 0.90
islandGlow.Color = CurrentTheme.Accent
islandGlow.Parent = IslandFrame
RegisterForRecolor(islandGlow,"Color")
RegisterAnimatedGlow(islandGlow, 0.82, 0.94, 0.85)

local islandStroke = Instance.new("UIStroke")
islandStroke.Thickness = 1.15
islandStroke.Transparency = 0.10
islandStroke.Color = CurrentTheme.Accent
islandStroke.Parent = IslandFrame
RegisterForRecolor(islandStroke,"Color")
RegisterAnimatedGlow(islandStroke, 0.04, 0.16, 0.70)

local IslandLogo = Instance.new("ImageLabel")
IslandLogo.Position = UDim2.fromOffset(7,6)
...
IslandText.Position = UDim2.fromOffset(45,1)
IslandText.Size = UDim2.fromOffset(65,40)
...
local function CreateSection(parent,key)
    ...
    label.Name = "Section_" .. tostring(key)
...
local function CreateToggle(parent,textKey,callback,default)
    local frame = Instance.new("Frame")
    frame.Name = "Toggle_" .. tostring(textKey)
    ...
    outerGlow.Transparency = 0.92
    outerGlow.Color = CurrentTheme.Accent
    outerGlow.Parent = frame
    RegisterForRecolor(outerGlow,"Color")
    ...
    local label = Instance.new("TextLabel")
    label.Name = "Label_" .. tostring(textKey)
    label.BackgroundTransparency = 1
    ...
    local controller = function(value)
        ...
        TweenService:Create(
            ...
            {
                Transparency = state and 0.05 or 0.42,
                Thickness = state and 2.2 or 1.2
            }
        ):Play()
        ...
        TweenService:Create(
            ...
            {
                Transparency = state and 0.58 or 0.92,
                Thickness = state and 8 or 6
            }
        ):Play()
        callback(state)
    end
...
local function CreateButton(parent,textKey,callback)
    ...
    button.Name = "Button_" .. tostring(textKey)
    ...
    RegisterForRecolor(outerGlow,"Color")
    RegisterAnimatedGlow(outerGlow, 0.84, 0.94, 0.75)
    ...
    RegisterAnimatedGlow(stroke, 0.24, 0.42, 0.90)
    ...
    button.Activated:Connect(function()
        ...
        TweenService:Create(glow,TweenInfo.new(0.10,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{BackgroundTransparency = 0.84}):Play()
        TweenService:Create(stroke,TweenInfo.new(0.10,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Transparency = 0,Thickness = 2.2}):Play()
        TweenService:Create(outerGlow,TweenInfo.new(0.10,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Transparency = 0.55,Thickness = 9}):Play()
        ...
        task.delay(0.12,function()
            if button.Parent then
                ...
                TweenService:Create(outerGlow,TweenInfo.new(0.22,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Transparency = 0.92,Thickness = 7}):Play()
                ...
                TweenService:Create(stroke,TweenInfo.new(0.18,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Transparency = 0.36,Thickness = 1.2}):Play()
            end
        end)
...
local function ShowIsland()
    ...
    IslandFrame.Size = UDim2.fromOffset(28,28)
