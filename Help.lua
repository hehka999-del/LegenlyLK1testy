...
==============================================================
          HUB - Troll & Universal
       LGK Premium Visual Overhaul v10.0
                   by Legenly
==============================================================
...
local Translations = {
    RU = {
        ...
        OrbitFling = "ORBIT FLING",
        VoidFling = "VOID FLING",
        ...
        FuckFront = "Выебать в рот",
        Victim = "Стать жертвой",
    ...
    EN = {
        ...
        SecGang = "Gang",
        FuckBack = "Back Attach",
        FuckFront = "Front Attach",
        Victim = "Victim",
...
local function Notify(title, message, color)
    ...
    bar.Size = UDim2.fromOffset(4, 74)
...
SplashLogo.Position = UDim2.fromScale(0.5,0.28)
...
SplashTitle.Position = UDim2.fromScale(0.5,0.46)
...
SplashSub.Position = UDim2.fromScale(0.5,0.525)
...
SplashStatus.Position = UDim2.fromScale(0.5,0.595)
...
MainBackground.ImageTransparency = 0.36
...
MainOverlay.BackgroundTransparency = 0.34
...
IslandFrame.Size = UDim2.fromOffset(150,42)
...
IslandLogo.Size = UDim2.fromOffset(32,32)
...
IslandText.Position = UDim2.fromOffset(43,0)
IslandText.Size = UDim2.new(1,-50,1,0)
IslandText.Text = "Troll | LGK"
IslandText.Font = Enum.Font.GothamBold
IslandText.TextSize = 11
...
local function StopAllActions()
    ...
    pcall(function()
        ...
        if char then
            ...
            local rs =
                (ru and ru:FindFirstChild("RightShoulder"))
                or
                (torso and torso:FindFirstChild("Right Shoulder"))
...
RunService.Stepped:Connect(function()
    pcall(function()
        ...
        then
            ...
            local rs =
                (rightUpperArm and rightUpperArm:FindFirstChild("RightShoulder"))
                or
                (torso and torso:FindFirstChild("Right Shoulder"))

            local speedMultiplier =
                TrollState.SpamSwordSpeed

            local rub =
                math.cos(
                    os.clock() * speedMultiplier
                )
        ...
        then
            local targetChar =
                TrollState.AttachTarget.Character
...
CreateSection(MainPage,"SecChat")
...
CreateButton(MainPage,"StopAll",function()
    ...
    Notify(
        "Stopped",
        Translations[CurrentLang].Stopped,
        Color3.fromRGB(255,255,255)
    )
end)
...
CreateButton(MainPage,"ThemeRed",function()
    ApplyTheme("Red")
    Notify("Theme",Themes.Red.Name,Themes.Red.Accent)
end)

CreateButton(MainPage,"ThemeBlue",function()
    ApplyTheme("Blue")
    Notify("Theme",Themes.Blue.Name,Themes.Blue.Accent)
end)

CreateButton(MainPage,"ThemePurple",function()
    ApplyTheme("Purple")
    Notify("Theme",Themes.Purple.Name,Themes.Purple.Accent)
end)

CreateButton(MainPage,"ThemeGreen",function()
    ApplyTheme("Green")
    Notify("Theme",Themes.Green.Name,Themes.Green.Accent)
end)

CreateButton(MainPage,"ThemeGold",function()
    ApplyTheme("Gold")
    Notify("Theme",Themes.Gold.Name,Themes.Gold.Accent)
end)
...
CreateSection(TargetPage,"SecDestroy")

CreateButton(TargetPage,"OrbitFling",function()
...
CreateButton(TargetPage,"VoidFling",function()
    if TrollState.TargetPlayer then
        RunOrbitFling(
            TrollState.TargetPlayer,
            true
...
CreateButton(TargetPage,"Freeze",function()
    if TrollState.TargetPlayer then
        TrollState.FreezeTarget =
            TrollState.TargetPlayer
...
CreateButton(TargetPage,"Unfreeze",function()
...
CreateButton(TargetPage,"SpinTarget",function()
    if TrollState.TargetPlayer then
        TrollState.SpinTarget =
            TrollState.TargetPlayer
...
CreateButton(TargetPage,"TPTarget",function()
    if
        TrollState.TargetPlayer
        and
        TrollState.TargetPlayer.Character
        and
        TrollState.TargetPlayer.Character:FindFirstChild(
            "HumanoidRootPart"
        )
    then
        local myChar =
...
CreateButton(TargetPage,"LoopKill",function()
    if TrollState.TargetPlayer then
        TrollState.LoopKillTarget =
            TrollState.TargetPlayer
...
CreateSection(TargetPage,"SecGang")

CreateButton(TargetPage,"FuckBack",function()
    StopAllActions()
...
CreateButton(TargetPage,"FuckFront",function()
    StopAllActions()
...
CreateButton(TargetPage,"Victim",function()
    StopAllActions()
...
CreateButton(TargetPage,"ClearTarget",function()
...
CreateSlider(
    ...
    TrollState.FOV,
    function(value)
        TrollState.FOV = value
    end
)

CreateSection(VisualPage,"SecSelf")

CreateToggle(VisualPage,"KillAura",function(state)
...
CreateInfoLabel(
    VisualPage,
    "Visual controls are client-side. Some experiences may overwrite Lighting or Camera values."
)
...
DiscordButton.Text = "https://discord.gg/vpfFGGjg9"
