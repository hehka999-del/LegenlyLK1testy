...
IslandFrame.Size = UDim2.fromOffset(190,46)
...
IslandLogo.Size = UDim2.fromOffset(36,36)
...
CreateButton(TargetPage,"Fling",function()
...
CreateSection(TargetPage,"SecTargetState")

CreateToggle(TargetPage,"Freeze",function(state)
    if state then
        if TrollState.TargetPlayer then
        ...
        TrollState.FreezeTarget = nil
        TrollState.FreezePos = nil
        Notify("Freeze", "Target unfrozen.", CurrentTheme.Accent)
    end
end,false)

CreateToggle(TargetPage,"SpinTarget",function(state)
    if state then
        if TrollState.TargetPlayer then
        ...
        TrollState.SpinTarget = nil
        Notify("Spin", "Target spin disabled.", CurrentTheme.Accent)
    end
end,false)

CreateToggle(TargetPage,"LoopKill",function(state)
    if state then
        if TrollState.TargetPlayer then
            TrollState.LoopKillTarget = TrollState.TargetPlayer
            TrollState.LoopKill = true
            Notify("Loop Kill", TrollState.TargetPlayer.DisplayName, Color3.fromRGB(255,0,0))
        else
            Notify("Target", Translations[CurrentLang].NoTarget, Color3.fromRGB(255,0,0))
        end
    else
        TrollState.LoopKill = false
        TrollState.LoopKillTarget = nil
        Notify("Loop Kill", "Disabled.", CurrentTheme.Accent)
    end
end,false)
...
-- White FX is intentionally client-side. A generic LocalScript cannot
-- guarantee replication to every other client without a game-specific server remote.
...
CreateInfoLabel(
    VisualPage,
    "White FX is a local visual effect. Universal replication requires server-side support from the specific experience."
)
