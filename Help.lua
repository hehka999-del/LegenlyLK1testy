...
local DISCORD_LINK = "https://discord.gg/jKqrr5Xss"

local function CopyToClipboard(text)
    local copied = false
    for _,fn in ipairs({setclipboard, toclipboard, writeclipboard}) do
        if typeof(fn) == "function" then
            local ok = pcall(function() fn(text) end)
            if ok then
                copied = true
                break
            end
...
local function ApplyTheme(themeKey)
    ...
    -- Re-render toggles after a theme change so OFF toggles stay dark
    -- while only ON toggles keep the animated accent glow.
...
SplashLogo.Position = UDim2.fromScale(0.5,0.285)
...
SplashTitle.Position = UDim2.fromScale(0.5,0.515)
...
SplashSub.Position = UDim2.fromScale(0.5,0.575)
...
SplashStatus.Position = UDim2.fromScale(0.5,0.655)
...
local function UpdateWindowSize()
    ...
    IsCompactLayout = UserInputService.TouchEnabled or viewport.X < 680

    if IsCompactLayout then
        -- Keep the whole hub inside phone screens with small margins.
        MainFrame.Size = UDim2.fromOffset(
            math.max(300, viewport.X - 12),
            math.max(310, viewport.Y - 18)
        )

        local sideWidth = math.clamp(math.floor(viewport.X * 0.27), 86, 108)
        Sidebar.Size = UDim2.new(0,sideWidth,1,0)
        Pages.Position = UDim2.fromOffset(sideWidth,0)
        Pages.Size = UDim2.new(1,-sideWidth,1,0)
...
IslandFrame.Size = UDim2.fromOffset(176,46)
...
IslandLogo.Position = UDim2.fromOffset(9,8)
IslandLogo.Size = UDim2.fromOffset(30,30)
...
IslandText.Position = UDim2.fromOffset(45,0)
IslandText.Size = UDim2.new(1,-54,1,0)
...
sidePadding = Instance.new("UIPadding")
sidePadding.PaddingTop = UDim.new(0,12)
sidePadding.PaddingLeft = UDim.new(0,10)
sidePadding.PaddingRight = UDim.new(0,10)
...
local function CreateToggle(parent,textKey,callback,default)
    ...
    glowGradient.Transparency = NumberSequence.new(0.35)
    ...
    local function render()
        ...
        glowStroke.Transparency = state and 0.10 or 1
        glowStroke.Thickness = state and 2.2 or 1
        TweenService:Create(
            switch,
    ...
    local controller = function(value)
        ...
        TweenService:Create(
            ...
            {
                Transparency = state and 0.10 or 1,
                Thickness = state and 2.2 or 1
            }
        ):Play()
        if state then
            task.spawn(function()
                local phase = 0
                while state and frame.Parent do
                    phase += 0.18
                    glowGradient.Rotation = phase % 360
                    TweenService:Create(
                        glowStroke,
                    ...
                    task.wait(0.55)
                    if not state or not frame.Parent then break end
                    TweenService:Create(
                        glowStroke,
                        TweenInfo.new(0.55,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),
                        {Transparency = 0.06}
    ...
    ToggleRenderers[textKey] = render
...
local function CreateButton(parent,textKey,callback)
    ...
    button.Activated:Connect(function()
        ...
        TweenService:Create(
            stroke,
            TweenInfo.new(0.10,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),
            {Transparency = 0.28, Thickness = 1.2}
        ):Play()
        task.delay(0.12,function()
            if button.Parent then
                TweenService:Create(button,TweenInfo.new(0.18,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{BackgroundColor3 = Color3.fromRGB(29,29,36)}):Play()
                TweenService:Create(stroke,TweenInfo.new(0.18,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Transparency = 0.70, Thickness = 1}):Play()
            end
        end)
        callback()
    end)
...
-- Forward declarations: the physics loop runs before the helper definitions below.
local EnableNoclip
local DisableNoclip
...
UserInputService.JumpRequest:Connect(function()
    if TrollState.Fly then
        TrollState.FlyVertical = 1
        task.delay(0.85,function()
            if TrollState.Fly and TrollState.FlyVertical == 1 then
                TrollState.FlyVertical = 0
            end
        end)
    end
...
local function ApplySmoothFly()
    ...
    if moveDir.Magnitude > 0 then
        -- Horizontal joystick movement stays horizontal. Looking up/down while
        -- moving also gives a small vertical component, making touch flight
        -- usable without requiring a keyboard.
        local horizontal = Vector3.new(moveDir.X,0,moveDir.Z)
        if horizontal.Magnitude > 0 then
            velocity = horizontal.Unit * TrollState.FlySpeed
            if camera then
                velocity += Vector3.new(0, camera.CFrame.LookVector.Y * TrollState.FlySpeed * 1.15, 0)
            end
        end
    end

    -- Keyboard and mobile jump button both request a clean vertical ascent.
    if UserInputService:IsKeyDown(Enum.KeyCode.Space) or hum.Jump or TrollState.FlyVertical == 1 then
        velocity += Vector3.new(0,TrollState.FlySpeed,0)
    end
...
ThemeDropdown.Size = UDim2.new(1,0,0,44)
ThemeDropdown.BackgroundColor3 = Color3.fromRGB(25,25,32)
ThemeDropdown.BorderSizePixel = 0
ThemeDropdown.ClipsDescendants = false
ThemeDropdown.ZIndex = 150
...
themeStroke.Transparency = 0.12
themeStroke.Color = CurrentTheme.Accent
themeStroke.Parent = ThemeDropdown
RegisterForRecolor(themeStroke,"Color")

local ThemeMain = Instance.new("TextButton")
ThemeMain.Name = "ThemeMain"
ThemeMain.Size = UDim2.new(1,0,0,44)
ThemeMain.BackgroundTransparency = 1
ThemeMain.BorderSizePixel = 0
ThemeMain.Text = ""
ThemeMain.AutoButtonColor = false
ThemeMain.ZIndex = 152
ThemeMain.Parent = ThemeDropdown

local ThemeDot = Instance.new("Frame")
ThemeDot.Name = "ThemeDot"
ThemeDot.Position = UDim2.fromOffset(14,11)
ThemeDot.Size = UDim2.fromOffset(20,20)
ThemeDot.BorderSizePixel = 0
ThemeDot.BackgroundColor3 = CurrentTheme.Accent
ThemeDot.ZIndex = 153
ThemeDot.Parent = ThemeMain
local themeDotCorner = Instance.new("UICorner")
themeDotCorner.CornerRadius = UDim.new(1,0)
themeDotCorner.Parent = ThemeDot

local ThemeMainText = Instance.new("TextLabel")
ThemeMainText.Name = "ThemeName"
ThemeMainText.BackgroundTransparency = 1
ThemeMainText.Position = UDim2.fromOffset(58,0)
ThemeMainText.Size = UDim2.new(1,-104,1,0)
ThemeMainText.Font = Enum.Font.GothamMedium
ThemeMainText.TextSize = 12
ThemeMainText.TextColor3 = Color3.fromRGB(235,235,240)
ThemeMainText.TextXAlignment = Enum.TextXAlignment.Left
ThemeMainText.Text = CurrentTheme.Name
ThemeMainText.ZIndex = 153
ThemeMainText.Parent = ThemeMain

local ThemeArrow = Instance.new("TextLabel")
ThemeArrow.Name = "ThemeArrow"
ThemeArrow.BackgroundTransparency = 1
ThemeArrow.Position = UDim2.new(1,-38,0,0)
ThemeArrow.Size = UDim2.fromOffset(28,44)
ThemeArrow.Text = "▼"
ThemeArrow.Font = Enum.Font.GothamBold
ThemeArrow.TextSize = 10
ThemeArrow.TextColor3 = Color3.fromRGB(175,175,185)
ThemeArrow.ZIndex = 153
ThemeArrow.Parent = ThemeMain

local ThemeList = Instance.new("ScrollingFrame")
ThemeList.Name = "ThemeList"
ThemeList.Position = UDim2.fromOffset(0,48)
ThemeList.Size = UDim2.new(1,0,0,0)
ThemeList.BackgroundColor3 = Color3.fromRGB(19,19,25)
...
ThemeList.ZIndex = 160
ThemeList.Parent = ThemeDropdown

local listCorner = Instance.new("UICorner")
listCorner.CornerRadius = UDim.new(0,11)
listCorner.Parent = ThemeList

local listStroke = Instance.new("UIStroke")
listStroke.Thickness = 1.4
listStroke.Transparency = 0.08
listStroke.Color = CurrentTheme.Accent
listStroke.Parent = ThemeList
RegisterForRecolor(listStroke,"Color")
...
listLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    ThemeList.CanvasSize = UDim2.fromOffset(0,listLayout.AbsoluteContentSize.Y + 14)
end)

local ThemeOrder = {"Red","Blue","Purple","Green","Gold"}
local ThemeDots = {}

local function SetThemeDropdownVisuals()
    ThemeMainText.Text = CurrentTheme.Name
    ThemeDot.BackgroundColor3 = CurrentTheme.Accent
    for key,dot in pairs(ThemeDots) do
        if dot and dot.Parent then
            dot.BackgroundColor3 = Themes[key].Accent
        end
    end
end
...
for _,key in ipairs(ThemeOrder) do
    ...
    option.Name = "ThemeOption_" .. key
    option.Size = UDim2.new(1,0,0, IsCompactLayout and 34 or 38)
    option.BackgroundColor3 = Color3.fromRGB(29,29,36)
    option.BorderSizePixel = 0
    option.Text = ""
    option.AutoButtonColor = false
    option.ZIndex = 161
    option.LayoutOrder = table.find(ThemeOrder,key) or 1
    ...
    dot.Name = "ColorDot"
    dot.Position = UDim2.fromOffset(12,9)
    ...
    name.Text = Themes[key].Name
    name.ZIndex = 162
    name.Parent = option

    option.Activated:Connect(function()
        ApplyTheme(key)
        SetThemeDropdownVisuals()
        ThemeList.Visible = false
        ThemeList.Size = UDim2.new(1,0,0,0)
        ThemeArrow.Text = "▼"
...
ThemeMain.Activated:Connect(function()
    themeOpen = not themeOpen
    ThemeList.Visible = true
    local optionHeight = IsCompactLayout and 34 or 38
    local wantedHeight = #ThemeOrder * (optionHeight + 5) + 12
    local maxHeight = IsCompactLayout and 220 or wantedHeight
    ThemeList.Size = themeOpen and UDim2.new(1,0,0,math.min(wantedHeight,maxHeight)) or UDim2.new(1,0,0,0)
    if not themeOpen then
        task.delay(0.18,function()
            if not themeOpen then ThemeList.Visible = false end
        end)
    end
    ThemeArrow.Text = themeOpen and "▲" or "▼"
end)

RegisterForRecolor(ThemeDot,"BackgroundColor3")
SetThemeDropdownVisuals()
