--[[
    HUB - Troll & Universal
    Разработчик: Legenly
    Версия: Hardcore Premium Edition v9.2 (Исправлены Fling, Freeze, позиционирование "Выебать в рот")
]]
...
-- ==========================================
-- FE ORBIT / VOID FLING (ИСПРАВЛЕННЫЙ)
-- ==========================================
...
RunService.Stepped:Connect(function()
    pcall(function()
            ...
            if targetChar then
                ...
                if thrp then
                    ...
                    elseif TrollState.AttachMode == "Front" then
                        -- Исправляем позицию: выше, чтобы был рот, а не живот
        ...
        -- ==========================================
        -- FREEZE (ВОССТАНОВЛЕННЫЙ ВИЗУАЛЬНЫЙ)
        -- ==========================================
        if TrollState.FreezeTarget and TrollState.FreezeTarget.Character then
            ...
            if tHrp then
                ...
                -- Принудительно удерживаем позицию и сбрасываем скорость
...
-- ==========================================
-- FE Kill Aura, Chat Spam, Loop Kill
-- ==========================================
task.spawn(function()
...
CreateButton(MainPage, "Unfreeze", function()
    if TrollState.FreezeTarget then
        StopAllActions() -- очищает всё, в том числе заморозку
